...
class IcnHost(threading.Thread):
    def __init__(self):
        ...
        self.max_beta_percentage = config.getfloat('TRADE', 'MAX_BETA_PERCENTAGE')
        ...
        self.timespan  = (self.send_auct_time - self.start_time).total_seconds()
    ...
    def _twap_rate_func(self, sym=''):
        ...
        if sym:
            if self._timespan[sym] <= 10.0:
                delta_t = 1.0
            else:
                delta_t = (curr_time - self._start_time[sym]).total_seconds() / self._timespan[sym]
        else:
            delta_t = (curr_time - _time_1500).total_seconds() / _timespan
        twap_rate = 0.0
        t1 = 5.0/9
        t2 = 8.0/9
        #t1 = 8.0/9
        if delta_t < 0.0:
            ...
            twap_rate = 0.25 * delta_t / t1
        elif delta_t < t2:
            twap_rate = 0.25 + 0.75 * (delta_t - t1) / (t2 - t1)
        else:
            twap_rate = 1.0
        return twap_rate

    def calc_beta_adj(self):
        max_beta_exposure = self.max_beta_percentage * self.max_global_notional
        ...
        for sym in self.symbol_list:
            ...
            if ref < 0.1:
                ...
                continue
            if self.pos[sym] > 0:
                long_beta_exposure += self.beta[sym] * self.pos[sym] * ref
            else:
                short_beta_exposure += self.beta[sym] * (-self.pos[sym]) * ref
        total_beta_exposure = long_beta_exposure - short_beta_exposure

        if total_beta_exposure > max_beta_exposure:
            long_scale = (short_beta_exposure + max_beta_exposure) / long_beta_exposure
            short_scale = 1.0
        elif total_beta_exposure < -max_beta_exposure:
            long_scale = 1.0
            short_scale = (long_beta_exposure + max_beta_exposure) / short_beta_exposure
        else:
            long_scale = 1.0
            short_scale = 1.0
    ...
    def update_target(self):
        ...
        long_scale, short_scale = self.calc_beta_adj()
        logger.info('Beta balancing (%f,%f).' % (long_scale, short_scale))
        ...
        for sym in self.symbol_list:
            ...
            if self.icn_factor[sym_id].pair > 0:
                imb_ratio = abs(self.icn_factor[sym_id].net) / self.icn_factor[sym_id].pair
                imb_ratio_scale = min(1.0, imb_ratio / 0.3)
            else:
                imb_ratio_scale = 1.0
            
            #_final_target_pos = int(self.partic[sym] * self.icn_factor[sym_id].side * self.icn_factor[sym_id].total)
            _final_target_pos = int(self.partic[sym] * self.icn_factor[sym_id].net * imb_ratio_scale)
            _final_target_pos = min(pos_limit, max(-pos_limit, _final_target_pos))
            _final_target_pos *= long_scale if _final_target_pos > 0 else short_scale
